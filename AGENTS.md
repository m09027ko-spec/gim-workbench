...
## 1モジュールずつ・小スコープ原則（事故防止の封筒）

モジュールは**1つずつ**追加する。エラーを減らす「封筒」はエージェント・作業場所に依存しない:

1. **コード前に確定 spec**。`docs/specs/<tool-id>.md` が数値・出典入りで「確定」になるまで実装着手しない。会話依頼で spec が無いときは Claude Code の `deep-research-intake` スキル（ユーザーの ChatGPT Deep Research 経由で数値を確定）に従う。
2. **1モジュール = 1ブランチ = 小さな差分**、`npm test` / `npm run build` 緑で**1コミット**。複数モジュールを1バッチにまとめない。
3. **アーキテクチャ変更は禁止**（絶対則7。必要ならユーザー確認＋ `docs/decisions.md` 記録）。

2026-05-17 の30ツール一括追加の事故は「確定仕様なしの大バッチ＋アーキ裁量」が原因。封筒を守れば事故率は下がる。担当振り分け（自宅=Claude Code 直 / 外出先=Codex リモート）と Codex へ渡すときのプロンプト必須項目は `docs/WORKFLOW.md` を参照。**Codex はこの節を必ず守る**（読み順に WORKFLOW.md が無いため、ここに要点を置く）。
